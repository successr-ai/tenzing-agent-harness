...
- Go 1.26.0+ (`mvdan.cc/sh/v3` requires it)
...
- **Agents can ask the user.** Connect mode adds an `ask_user(question)` tool: the question goes to the plane as `input_request`, the turn blocks, and the plane's `answer` comes back as the tool's result. Escalated tool calls go to the plane as `approval_request` and wait for its decision when `approval_timeout` is set (the default, 0, denies them immediately).
