...
- `Text`: `Printer.Print` of a copy of the enclosing `Stmt` (redirects included, `&`/`!` dropped) whose `Cmd` is the `CallExpr` with `Assigns` cleared; `Raw`: the same with assignments. (Printer normalises whitespace: `grep  -n` → `grep -n`, `> out` → `>out`; backticks print as `$(…)`. Document in `AGENTS.md`.) See Amendment.
...
5. **Rewire `BashRules`** (`bash.go`, `permissions.go`, `bash_test.go`, `permissions_test.go`). Delete the old splitter and its tests; port the existing `Verdict` tests to the new signature — every existing allow/deny expectation must still hold (`TOKEN=x go get` covered by `go *`; `LD_PRELOAD=*` deny hits the raw form; `a && b` with `b` denied → Deny; `ls 2>/dev/null` matched by `ls *`, but `ls > f` NOT matched by `ls *` — see Amendment). New tests: read-only auto-allow (`grep x f | head` with empty rules → Allow ok); mixed (`ls; rm x` empty rules → AskUser, reason names `rm x`); category deny (`categories.deny: [fs:delete]`, `rm x` → Deny); category allow (`categories.allow: [vcs]`, `git commit -m x` → Allow; `git push` → AskUser because net not allowed); glob allow beats category deny (`allow: [rm *]`, `deny cat fs:delete`, `rm x` → Allow); parse error → AskUser with reason, and parse error + `deny: [rm *]` + `rm -rf / 'x` → Deny; session glob covers but `Lists()` excludes it; `UnmarshalJSON` rejects `"unknown"` and bad class names. → verify `go test -race ./internal/features/permissions/...`.
...
## Amendment (2026-09-13, after implementation)

Two statements below were wrong and the implementation does not follow them:

1. **A binary glob does NOT cover a redirect write.** Step 5 says `ls > f` must stay matched by `ls *`, "so existing settings files keep meaning what they meant". That lets any read-only binary's glob allowlist arbitrary file creation (`cat *` → `cat > anything <<EOF`). Implemented behaviour: an allow glob covers the binary it names, never a redirect bolted onto it. A segment whose `fs:write` comes from a redirect is covered only when the matching glob itself contains `>` (`cat >*`, `rm * >*`) or `fs:write` is in `categories.allow`. Descriptor dups (`2>&1`) and `/dev/null` discards are not writes and stay covered. Deny globs are unchanged and still see the redirect in `Text`. `Suggest` still proposes no glob for redirect writes (`snapshot.covered` in `bash.go`).
2. **`Text` is printed from the `Stmt`, not the `CallExpr`** (Segment construction below says `CallExpr`), so redirects appear in `Text` — `cat >f`, no space — for deny globs to target and allow globs to spell. `&`, `!`, and `|&` decorations are dropped.
