...
- **Connect-only:** `--connect-token` (bearer token on the upgrade request; env `TENZING_CONNECT_TOKEN`), `--connect-backoff` (reconnect delay base, default 1s, doubling to a 30s cap; tenzing.yaml `connect.backoff`). The `--connect` flag itself is a mode switch; its env (`TENZING_CONNECT`) and tenzing.yaml `connect:` are the config-file equivalents (see "Connect mode" below)
