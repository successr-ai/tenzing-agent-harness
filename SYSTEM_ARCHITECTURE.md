...
Go module: `github.com/successr-ai/tenzing-agent-harness` (go 1.26.0 — required by `mvdan.cc/sh/v3`)
...
**Risk changes the process.** The permissions extension (`internal/features/permissions`, a `core.ToolCallHook` registered FIRST in the default extension order) classifies each tool call: `Deny` > `Ask` > `Allow` > policy default, matched case-insensitively by tool name plus origin prefixes. The default policy asks for anything that executes code or writes files (`bash`, `write`, `edit`, `repl`, `spawn_agent`) and for every MCP-origin tool (`AskOrigins: ["mcp:"]`); everything else — including the read-only, gate-mandated `advisor` — is allowed. `AskUser` makes the core loop emit `ApprovalRequestedEvent` and block (up to `ApprovalTimeout`, harness default 120s; timeout/cancel = deny) for a `Respond(bool)` — cmd/app forwards it over SSE as `approval.requested` and resolves it via `POST /approve {call_id, approved, allow?, scope?}` with approve/deny/allow-always/allow-for-session buttons in the embedded UI. For `bash` the name-level Ask is refined per command by `Policy.Bash` (`internal/features/permissions/bash.go`): `internal/features/permissions/shell` parses the line with `mvdan.cc/sh/v3` (Bash dialect — the tool runs `bash -c`) into the simple commands it executes (through pipes, chains, substitutions, compound commands, wrappers like `sudo`/`xargs`/`find -exec`, and literal `bash -c`/`eval` scripts) and classifies each as `read`, `fs:write`, `fs:delete`, `net`, `vcs` or `unknown` from a built-in command table plus redirect analysis; `Verdict` then applies, in order, settings.json deny globs, allow globs (persisted or session-only), category deny, category allow — with fully read-only commands auto-allowed and unknown ones always asking — and the analysis summary rides `ApprovalRequestedEvent.Reason` to the UI. Configure with `harness.WithPermissionPolicy`, opt out with `harness.WithPermissionsDisabled` (headless/trusted drivers), tune waiting with `harness.WithApprovalTimeout`. `harness.WithReadOnly` (CLI `--read-only`) is the no-approver mode: it REPLACES the permissions extension with a `readOnlyExt` hook (`internal/harness/readonly.go`, shared with subagent loops) that denies every call whose tool is not marked read-only per `Composite.ReadOnly` (unmarked/unknown = mutating), except a `bash` command the shell analysis classifies entirely `read` against the built-in table, which runs — `WithPermissionPolicy` is ignored, no `AskUser` escalation exists (MCP tools' own read-only claims are trusted), and the approval timeout is forced to 0 so no prompt ever fires. `advisor` is marked read-only (pure LLM call, `internal/features/advisor`); `repl` is marked read-only (its sandbox blocks file writes — only shared in-memory blackboard state mutates); `spawn_agent` is exempt by name because child loops carry the same gate — spawned agents cannot mutate the filesystem, only shared in-memory blackboard state. The check happens in the core loop before tool execution — not in the tool itself, not in the Agent.

**Long tasks have budgets.** The budgets extension (`internal/features/budgets`, a `core.BeforeIterationHook`) enforces per-turn limits — `MaxIterations`, `MaxWallClock`, `MaxTokens` (input+output cumulative); zero = unlimited. The loop populates `TurnContext.Elapsed/InputTokens/OutputTokens` before each iteration's hooks; when a limit trips the extension sets `Terminate` and the turn ends gracefully with `TurnResult{Terminated: reason}` — a result, not a crash. Wire via `harness.WithBudgets(limits)`; subagents get `MaxIterations` from `WithSubagentMaxIterations` (default 100) through the same extension. Cost budget (USD) is deferred at the harness level — pricing now exists in the app layer (tenzing.yaml per-MTok model costs feed the API server's `costs.Tracker`, `api/costs`), but `budgets.Limits` has no `MaxCost` field yet.
...
cmd/app/root.go                         cobra root command `tenzing` — flags, tenzing.yaml load, model precedence, dispatches to print or serve mode
...
internal/app/modelregistry/             Model registry + client factory — tenzing.yaml providers:/models: → Registry (Build/Resolve/ResolveSystemOne/Names/List/Pricing),
                                        Factory.Get client construction, MergeProviderFlags
internal/app/wsclient/                  Control-plane WebSocket client — dial/handshake/pumps/backoff; disconnect cancels the in-flight turn
                                        (agent side of docs/adrs/2026-09-12-control-plane-fleet/PROTOCOL.md)

api/api.go                              api.Server + api.ServerConfig + collaborator interfaces (Harness, EventBus, Nexus, LogStream, AllowStore) —
                                        New/Attach/Start/Shutdown, route table (/query, /cancel, /steer, /state, /approve, /preview, /suggest,
                                        /sessions, /messages, /compact, /thinking, /model, /models, /stats, /trust, /clear, /resume, /info,
                                        /debug, /ingest/{name})
api/events.go                           forwardEvents — bus → subagent labels, approval capture, cost tracking, SSE publish
api/turns.go                            /query /steer /state /cancel /info, image validation, nexus wake prompt
api/approvals.go                        /approve /preview /suggest handlers
api/controls.go                         /clear /resume /compact /thinking /model /models /stats /trust
api/sessions.go                         /sessions (GET/DELETE/PATCH) /messages
api/turnqueue/                          turnqueue.Queue — one turn at a time, FIFO follow-ups, cancel/close
api/sse/                                sse.Broadcaster (fan-out to SSE clients, drop-on-full) + sse.Translate (bus event → wire envelope)
api/approvals/                          approvals.Registry — pending AskUser calls by call ID
api/costs/                              costs.Tracker — token + USD accounting from LLMResponseEvents (pricing from tenzing.yaml model costs)
api/ui/                                 Embedded chat UI (ui.Handler; single-page HTML served at /; image attachments, live cost display, --debug-gated verbosity, Markdown-rendered answers)

internal/app/trust.go                   Project trust gate — trust.json persistence, ResolveProjectTrust/SetProjectTrust
internal/app/bashallow.go               BashAllowStore — persists "allow always" globs to settings.json (allow/deny/categories/classify; session grants excluded), LookupTool
internal/app/logsse.go                  LogBroadcaster — slog tee to the /debug SSE stream
...
│   ├── advisor/                        Tool + write-gate ext — transcript-aware LLM consult,
│   │                                    registered only via WithAdvisorLLM
...
│   ├── permissions/                    ToolCallHook — name/origin policy gating (default-on, runs first); BashRules per-command globs + categories
│   │   └── shell/                      Pure bash analysis on mvdan.cc/sh: Analyze → Segments classified read/fs:write/fs:delete/net/vcs/unknown
...
│   │                                    escalate-only, two batched Evaluate calls per iteration); harness.WithSystemOne
...
Six states, six transitions (`internal/core/fsm.go`). `Reset` can fire from any state, including `started` — `RunTurn` itself resets first thing, before the loop's first iteration. FSM is per-`Loop` instance. `TransitionStates` strips cancellation from the context before applying a transition (`context.WithoutCancel`): transitions are in-memory bookkeeping with no I/O to abort, and looplab/fsm abandons one whose context is done *without clearing the pending transition*, so every later transition — the terminal `Reset` included — fails with "previous transition did not complete" and the loop is wedged for the process's life (`Transition()` cannot recover it: it re-checks the same dead context).
...
- `WithSubagentLLM` / `WithBlackboardLLM` / `WithAdvisorLLM` — per-role `common.LLM` clients; unset subagent/blackboard roles fall back to `mainLLM`. `WithAdvisorLLM` has no fallback: it is the opt-in switch that registers both the transcript-aware advisor tool (reads the main conversation from the context store at call time) and the advisor write-gate extension (each turn's first state-changing tool call is denied until `advisor` is consulted; plan checkpoints — `TodoWrite`, `TodoCreate`, `TodoUpdate` to `done` — are denied unless `advisor` was called since the last state-changing call; read-only tools flow freely; after `WithAdvisorCadence(n)` iterations without a consult (default 6, -1 off) every tool is denied until the next consult; ≤`WithAdvisorMaxCalls(n)` consults/turn, default 30). `WithAdvisorNudge(n)` adds an unconsulted-executor reminder from iteration n (default off). `WithBlackboardLLM` sets the client used for `llm_query`/`llm_batch` sub-LM calls inside the shared blackboard REPL.
...
- `WithSystemOne(cfg, resolveModel)` — registers the System One decision model extension (`internal/features/systemone`): a `ToolBatchHook` gating tool calls, a `BeforeIterationHook` judging advisor need and routing the turn's model. The gate runs before the per-call permissions and advisor-gate hooks, as every batch hook does, and the strictest decision wins (escalate-only); `cfg.Client` nil registers nothing; `resolveModel` (alias → `common.LLM`) applies a routing choice through `switchLLM`. Two `Evaluate` calls per iteration, fail-open throughout.
- `WithThinking` / `WithThinkingBudget` / `WithLLMRetry` / `WithCompressionThreshold` / `WithCompressionKeepMessages` — reasoning toggle, per-call reasoning token cap (exact on Anthropic, tiered/level-mapped on OpenAI-compatible and Ollama; beats a model entry's `reasoning_effort`), transient-error retry policy, and compression tuning for the default brain/context store.
...
The harness holds no LLM cache — it uses exactly the clients injected via `New`/`With*LLM`/`SetLLM`. cmd/app caches clients per (provider, model, reasoning effort) in `internal/app/modelregistry`'s `Factory`.
...
`harness.New` (`internal/harness/harness.go`) → resolve models → `skills.NewRegistry()` + `RegisterSkillDir` per configured dir → build shared `blackboard.New(...)` (unless disabled) → assemble default extensions in order: permissions (unless disabled), advisor write-gate (if `WithAdvisorLLM`), reminders, budgets (if limits set), mcp (if servers configured), `blackboard.NewExt(bb, "main")`, `subagent.NewSpawnExt(factory)` (if depth > 0), then user `WithExtension`s → `toolport.NewRegistry(cwd)` seeded with `builtins.Defaults()`, `RegisterFromProvider(todoFile)`, advisor tool (if `WithAdvisorLLM`, wired to the main context store's `Messages`), user `WithTool`s → render system prompt (`features/prompts` base + extension `PromptFragments`) → `runner.NewAgentRunner(...)` → `core.NewLoop`.
...
| `advisor` | only with `WithAdvisorLLM` | Strategic review by stronger model | One-shot call to the advisor model; automatically sees the full main conversation (optional `question` argument). Paired with a write-gate: each turn's first state-changing call requires a prior consult. Not given to subagents |
...
Hooks: `eventbus.Hooks` struct has one typed `func(XxxEvent)` field per event type (all optional). `eventbus.StartHooks(bus, hooks)` subscribes with a buffer of 64, dispatches in a goroutine, and returns a `stop func()` that unsubscribes *and waits for the queued events to be dispatched* — `Unsubscribe` closes the channel, so the loop drains before exiting, and nothing emitted just before the call is lost. `Harness.Shutdown` calls it to stop hook dispatch; `rotateSession` relies on the drain so a session record is complete before `/clear` or `Resume` closes it.
...
- **Mode:** `-p/--prompt`, `--output-format` (`text` default, or `json` for JSONL events), `--list-models`, `--connect <ws-url>` (control-plane mode; mutually exclusive with `-p`)
- **Models:** `--model` (default `ollama/glm-5.2:cloud`), `--subagent-model`, `--blackboard-model`, `--advisor-model` (enables the advisor tool + write-gate; `--advisor-nudge N` optionally reminds an unconsulted executor from iteration N; `--advisor-cadence N` forces a consult every N iterations, default 6; `--advisor-max-calls N` caps consults per turn, default 30) — resolved via `cfg.deps`' registry (see "Model registry" below)
- **Budgets:** `--max-turn-tokens`, `--max-iterations`, `--max-wall-clock`
- **Toggles:** `--subagent-depth` (default 1), `--approval-timeout`, `--no-permissions`, `--read-only` (deny tools not marked read-only, no approval prompts), `--thinking` (tri-state: only applied when explicitly passed), `--thinking-budget` (reasoning tokens per LLM call; 0 = provider default; tenzing.yaml `thinking_budget:`), `--no-session`, `--no-context-files`
- **Prompt / sessions / trust:** `--system <file>` (file contents replace the system prompt; wins over SYSTEM.md overrides), `--system-prompt <text>` (literal text replacing the system prompt; wins over `--system`, whose file is then not read), `--resume <id>`, `-c/--continue` (latest session for cwd; mutually exclusive with `--resume`, requires persistence), `--trust` (treat cwd as trusted this run, not persisted), `--timeout` (print-mode turn deadline; warned-and-ignored in serve mode)
- **Wiring:** `--mcp-server` (repeatable, `name=command arg1 arg2`), `--conversation-id`
- **Serve-only:** `--port` (default 8080, env `SERVER_PORT`), `--nexus-config` (default `nexus.yaml`, env `NEXUS_CONFIG`) — unused by print or connect mode; passing them with `-p` (or in connect mode) prints a stderr warning
- **Connect-only:** `--connect-token` (bearer token on the upgrade request; env `TENZING_CONNECT_TOKEN`), `--connect-backoff` (reconnect delay base, default 1s, doubling to a 30s cap; tenzing.yaml `connect.backoff`), `--connect-ephemeral-grants` (default true: runtime-approved bash globs stay in memory; false persists them to settings.json; env `TENZING_CONNECT_EPHEMERAL_GRANTS`; tenzing.yaml `connect.ephemeral_grants`). The `--connect` flag itself is a mode switch; its env (`TENZING_CONNECT`) and tenzing.yaml `connect:` are the config-file equivalents (see "Connect mode" below)
...
One YAML file for every durable setting — providers and models (the registry lives here; replaces the former `models.yaml`), advisor, subagent, budgets, permissions, connect, MCP servers, serve settings. The file is **required**: nothing is compiled in, so a missing one is a startup error pointing at `tenzing init`. `internal/config` owns the schema (`config.File`) and `Load`: strict decoding (unknown keys are startup errors), Go-duration strings, pointer fields for scalars whose zero is a meaningful value (`subagent_depth`, `approval_timeout`, `thinking`, `port`) so the merge can distinguish "set to zero" from "omitted". Located via `--config` > `TENZING_CONFIG` env > `./tenzing.yaml` (`resolveConfigPath` in `cmd/app/configmerge.go`); missing is a startup error either way — named paths report the path, the probed default points at `tenzing init`. Removed keys carry migration hints (`migrationHint`): `models.entries`/`models.default` → the top-level `models:` list, `base_url`/`api_key` → `providers[]`.

### Model registry (`internal/app/modelregistry`, wired by `cmd/app/deps.go`)

`modelregistry.Build(providers, models)` (pure, no I/O — `root.go` loads tenzing.yaml, merges `--provider` JSON flags over its `providers:` list by name via `modelregistry.MergeProviderFlags`, then `buildDeps` passes both lists) builds the per-run registry + client `Factory`, parked on `cfg.deps` before any resolution — there are no process globals. A **provider** is a labelled backend: `name` (unique label, referenced by models, and what the client reports itself as in logs), `type` (the wire protocol — `anthropic`, `ollama`, or `openai_compat`, defaulting to `openai_compat` when absent; any unrecognized value is a startup error, vendor names included — a hosted OpenAI-compatible API is `openai_compat` plus its `url`), `url` (required for `openai_compat`, optional for the other two whose clients know their vendor endpoint — note ollama's default is the **cloud** endpoint, not localhost), optional `api_key` (empty = no auth; `$VAR` expands from the environment), and optional `extra` (a map injected into every request body by dotted path; `openai_compat` only, ignored elsewhere, applied in sorted key order, with `max_completion_tokens` reserved as the parameter-rename client option). Declare a type twice to reach a local box and a hosted endpoint from one config. A **model** is `name` (the local alias every ref uses), `provider` (a declared provider name), `model_name` (the wire id sent to the provider), plus `context_window`/`max_response_tokens` (default 128k/32k; the latter caps one response, unlike the file's top-level `max_turn_tokens` which bounds a whole turn), `vision`, `reasoning_effort` — carried on `common.ModelDefinition` and translated by the factory's `buildLLM` into `reasoning_effort` (OpenAI-compatible) or the `think` level (Ollama), passed through verbatim and part of the `Factory` cache key, though a request `ThinkingBudget` (`--thinking-budget`) beats it on both; Anthropic warns and ignores it — and `cost` (USD per MTok, cache read/write defaulting to the Anthropic 0.1x/1.25x convention, keyed by lowercase `model_name` because that is what `LLMResponseEvent.Model` carries). Resolution takes the alias; `provider/name` refs and the compiled-in `models.Standard()` set are gone. Effective main-model precedence: `--model` > tenzing.yaml `model:`, with no model at all a startup error.

`models:` is a mapping of two kinds — `llm:` (everything above) and `systemone:` (decision models). A bare sequence is read as `llm:`, so pre-split configs keep working (`config.ModelsSection.UnmarshalYAML`, which re-runs each branch through a strict decoder because `yaml.Node.Decode` drops `KnownFields`). A `systemone:` entry is `name`/`provider`/`model_name` only: the answer is a fixed-size typed value (nothing to cap, output unbilled), the model takes no images and has no reasoning tier, and its token budgets are constants in the protocol package rather than config. Its provider must be `type: systemone` and an `llm:` entry's must not — both directions are startup errors, as is one alias appearing in both lists (refs are bare aliases, so the namespace is shared). `Registry.ResolveSystemOne(alias)` returns a `ResolvedSystemOne{Name, Provider}` — deliberately not a `common.ModelDefinition`, whose fields all describe chat models — and `Factory.GetSystemOne` builds a `common.SystemOne` from it, cached per (provider name, wire name) in a cache of its own. `deps.judges` (`cmd/app/deps.go`) is the seam, and `--model` cannot name one. `systemone_model:` (an alias, never an inline definition) names the entry that drives `internal/features/systemone` — tool gating, advisor consults, model routing — and the optional `systemone:` block tunes it: `recent_messages`, plus `gate`/`advisor`/`routing` sub-blocks each with `enabled` and their probability thresholds. Every sub-setting is a pointer so "omitted" keeps the feature's own default while an explicit zero survives. Validation rejects a block without the ref, a ref naming a chat model or nothing at all, a threshold outside [0,1], and `routing.enabled: true` with fewer than two `models.llm:` entries carrying a `description:` — that key is the routing candidate list, and it enables routing by itself when two or more exist. `harness.WithSystemOne(cfg, resolveModel)` registers the extension and binds its four late dependencies, and `cmd/app/systemone.go` (`systemOneOption`) builds both from these keys — client via `deps.judges`, candidates via `routingCandidates`, resolver via the registry. `--systemone-model` is the flag; the block has none. `--list-models` appends `systemOneSummary`: the decision model in play and what routing may choose between.

### Connect mode (control-plane dial-out, `internal/app/wsclient`)

The third run mode (`--connect <url>`, or tenzing.yaml `connect:`): the process makes an **outbound-only** WebSocket connection to a control plane (whose server side is external — `docs/adrs/2026-09-12-control-plane-fleet/PROTOCOL.md` is the normative, versioned contract). `runConnect` (`cmd/app/connect.go`) wires logging, trust/project config, `harnessOptions`, and the harness — no `api.Server`, no HTTP listener, no nexus (its trigger target is serve mode's `srv`; re-adding waits for a turn-injection seam). The wsclient owns the connection lifecycle: dial with bearer token + `tenzing.v1` subprotocol, `hello`/`welcome` handshake (protocol version "1", capabilities, `last_turn` when a turn was in flight at the previous disconnect), read pump dispatching commands (`query`/`steer`/`cancel`/`approve`/`set-model`/`set-thinking`/`shutdown`) and a write pump draining the outbox, jittered exponential backoff reconnecting forever (transient) vs. exiting non-zero (config/startup errors). **A dropped connection cancels the in-flight turn**: each turn's context derives from the connection's, so socket loss aborts the turn via the loop's clean "turn canceled" outcome (`internal/core/loop.go` — no ErrorEvent, FSM reusable), the outcome classifies as `cancelled_disconnect` and rides the next `hello`'s `last_turn` (advisory; the disconnect itself is what the plane acts on), and the event forwarder stops forwarding once no turn is running — the backlog-discard rule. Explicit `cancel` commands cancel the turn context only (`cancelTurnContext`), classifying as `cancelled`. One turn at a time with FIFO follow-ups (`connectSerialQueue`, sync.Cond-based); `cancel` and disconnect both flush waiters (they report `cancelled` — never carried across a reconnect; the plane infers from the disconnect). Approvals are push: `ApprovalRequestedEvent` responders are captured into an `approvals.Registry` (the `api/events.go` pattern) and answered by `approve` commands on the same socket; an `approve.glob` goes through `connectApprove` — `BashRules.AllowPatternSession` in memory when `cfg.ConnectEphemeralGrants` (the default; per-node grants die with the process) or the command says `scope: "session"`, else `BashAllowStore.Add` (settings.json, serve parity). Event forwarding mirrors `api/events.go`'s observe (approval capture, subagent labels) and pushes `wire.ToWire` envelopes tagged with the running turn's correlation id; it also feeds `turnStats` — the per-turn tally reset at turn start and on disconnect — with denials (`result.denied_tools`) and the `file_path` of every successful Read/Edit/Write call (`result.files_touched`, deduplicated, first-seen order). `RunTurn` hands the wsclient a `wsclient.TurnReport` (answer, error, denied, files) and the client builds the `result` from it. `hello.conversation_id` is `Harness.ConversationID()` (empty when persistence is off) so the plane knows what to `--resume`. Teardown: a `shutdown` command sets the client's `shuttingDown` flag, cancels the turn context and flushes the serial queue (the existing `Cancel` handler), waits for the turn's `result` to be queued (`turnDone`), then the write pump drains the outbox, writes `shutdown_ack`, closes normally, and `Run` returns nil instead of reconnecting — `runConnect` returns, the deferred `Harness.Shutdown()` flushes session persistence, exit 0. `main.go` traps SIGTERM alongside SIGINT, so a plane's kill signal takes the same path minus the ack. `deps.llms` is the `llmSource` interface (`*modelregistry.Factory` in production) so `TestConnectResumeEndToEnd` (`cmd/app/connect_e2e_test.go`) can drive `runConnect` twice against an in-process plane double with a scripted LLM and prove `--connect --resume` restores history. The headless permission default applies (deny mutating tools instantly unless `--approval-timeout`/`--no-permissions`/`--dangerously-skip-permissions` was set).

### Setting precedence

Per setting: explicit flag > env var > tenzing.yaml > default. `mergeEnv` applies the three pre-existing env vars (`SERVER_PORT`, `LOG_DEBUG`, `NEXUS_CONFIG`) plus the connect env vars (`TENZING_CONNECT`, `TENZING_CONNECT_TOKEN`): an explicitly-passed flag always wins, otherwise the env var overrides the flag default. `mergeConfigFile` (`cmd/app/configmerge.go`) then fills from tenzing.yaml whatever neither flags nor env set — including the `*Set` markers for the unset-vs-zero fields, `mcp_servers` entries appended pre-parsed to `cfg.MCPServerConfigs` (additive with `--mcp-server` flags), and the `connect:` section into `cfg.ConnectURL`/`ConnectToken`/`ConnectBackoff`. One further env-only setting loads via the same `Config` struct: `TENZING_PROJECT_TRUST` (default trust decision, see "Project config & trust" below).

### Project config & trust

Project-local drop-in config — `./SYSTEM.md` (replaces the system prompt), `./APPEND_SYSTEM.md` (appends to the rendered default), `./.tenzing/prompts/` (slash-command templates) — is consumed only for trusted directories (`internal/app/trust.go`, `cmd/app/projectconfig.go`). Decisions persist per directory in `<UserConfigDir>/tenzing/trust.json`; absent a persisted decision, `TENZING_PROJECT_TRUST` decides (`trust` grants, anything else — the default `skip` — denies). `--trust` grants for one run without persisting; serve mode also exposes `GET /trust` (decision + source: `persisted`/`env`/`default`/`error`) and `POST /trust` (persist; applies on restart since config is read at startup). Global equivalents under `<UserConfigDir>/tenzing/` (`SYSTEM.md`, `APPEND_SYSTEM.md`, `prompts/`) always load. Precedence: project beats global, replace (SYSTEM.md) beats append (APPEND_SYSTEM.md), and an explicit `--system` or `--system-prompt` beats both (project-config options are applied before `harnessOptions`, so the later option wins). Template dirs register global-first so project names win on collision. AGENTS.md context files are exempt from trust.
...
`Server.forwardEvents` (`api/events.go`) forwards a subset of bus events to `/events` SSE clients: `sse.Translate` (`api/sse/translate.go`) maps each forwarded event to `wire.ToWire`'s envelope wrapped in an `sse.Envelope` that adds a server-side `agent` label (subagent runner-id → blackboard slot name) — the SSE event name is the wire type (`tool_execution.started`, `tool.succeeded`, `tool.failed`, `llm.response`, `tool.progress`, `approval.requested`, `subagent.started`, `subagent.stopped`, `steering.injected`, `llm.retry`, `model.changed`, `thinking.changed`, `images.attached`, `nexus.channel_error`, `nexus.channel_status`, `nexus.trigger`). Event types outside that set are not forwarded. `forwardEvents` keeps the side effects the pure translation can't own: the subagent label map, approval-responder capture, and cost accounting — every `llm.response` is tracked by the `costs.Tracker` and followed by a `cost` SSE event carrying the running `costs.Stats`. Server-lifecycle SSE events (`status`, `answer`, `error`, `queued`, `cost`, and raw-text `text_delta`/`thinking_delta`) are emitted directly by the server, not via wire. The embedded chat UI (`api/ui/index.go`) parses the wire envelope shapes (`.data` fields plus top-level `.agent`) and adds image attachments (paste/drag-drop, gated on `/state`'s `vision`) and a live cost display. Verbosity is gated on the server's `--debug` state, substituted into the page's `DEBUG` constant by `ui.Handler` (`__DEBUG__` placeholder): without it thinking collapses to one `✻ Thinking…` line, tool calls render as `⏺ name(input)` followed by a `⎿  N lines` / `⎿  error: …` outcome line, and `tool.progress` detail is suppressed (the status bar still updates); with it the UI streams full thinking text and the first 10 lines of each tool result. Slash commands are handled client-side: `COMMANDS` pairs each name with the endpoint call it makes, `parseCommand` splits a submitted line, and `matchCommands` drives a type-ahead menu (arrow keys, Tab/Enter to accept, Escape to dismiss; argument-less commands submit on accept). `/clear` and `/resume` also reset the local transcript, token counters and cost. Anything not in `COMMANDS` — including an unknown `/name` — is posted to `/query` untouched, so `prompttmpl`'s file-based templates keep working. The composer autosizes (`autosize`): two rows until the text needs a third, growing to a seven-row ceiling and scrolling past it, measured from `scrollHeight` against the line height so soft-wrapped rows count the same as newlines (Enter still sends, Shift+Enter still inserts a newline). It also keeps an in-memory input history (`history`/`historyKey`) of every submitted line — queries, slash commands, steers, consecutive duplicates collapsed — that Up/Down walk Claude Code-style: Up recalls only with the caret on the first line and Down only on the last, the draft typed before the first Up comes back on the final Down, edits to a recalled entry are discarded, and the slash menu's own arrow handling wins while it is open. The status bar (`renderStatus`) shows the conversation ID (`⟲ <id>`, the `--resume` handle, from `/state`) and a 20-cell context gauge — `[████░░░░] 41%  82k/200k`, green until 70%, yellow to 90%, red past it — driven by the main agent's last `llm.response` prompt size (`input_tokens` plus the cache fields Anthropic reports separately) over `/state`'s `context_window`; a 0 window hides the gauge. Mid-turn it carries a note (`thinking…`, or the running tool's `tool.progress` phase) and withholds the still-moving cumulative token/cost totals. Tool calls render Claude Code-style as `Tool(primary argument)` (`toolArg`/`toolCall`): the identifying argument is picked from a per-builtin table (`bash`→`command`, `Read`/`Write`/`Edit`→`file_path`, `Glob`/`Grep`→`pattern`, `ls`→`path`), extension/subagent/MCP tools fall back to the input object's first key, and non-object input is shown verbatim; paths shorten against `/state`'s `cwd` then `home`. Approval prompts show the same argument untruncated-to-500 with newlines intact, so the decision is made on the real command. Assistant answers are Markdown-rendered by a ~30-line dependency-free `renderMarkdown` (fenced/inline code, bold, italic, headings, ordered/unordered lists, http(s) links): the source is HTML-escaped and code fences are stashed before any tag is emitted, so model output can never inject markup, and rendering runs once when a stream finalizes rather than per delta.
...
Cancellation classification: a `loopErr` that unwraps to `context.Canceled` is reported as `turn canceled` with an info-level log and **no** `ErrorEvent` — the caller stopped the turn, which is an outcome rather than a fault. `context.DeadlineExceeded` (wall-clock budget, `--timeout`) stays a failure and keeps its `ErrorEvent`. Serve mode mirrors this: `runTurn` broadcasts a `canceled` SSE instead of `error`, which the UI renders as a dim `⊘ canceled` line.

| `POST /clear` | → `Harness.Clear` — drops history, rotates to a new conversation ID, resets the cost tracker (409 mid-turn) |
...
| `POST /model` | `{model}` (a declared model alias, or inline JSON; resolved via the registry) → `Factory.Get` → `Harness.SetLLM` |
...
`NewAppContainer(cfg *cliConfig)` (`cmd/app/container.go`) resolves the model (registry already loaded by `root.go`), resolves project trust (`--trust` short-circuits; else trust.json / `TENZING_PROJECT_TRUST`), loads the drop-in project config, then wires options in winning order — project config, registry base URLs, `harnessOptions(cfg)`, nexus tools — and builds the HTTP layer by dependency injection: `api.New(api.ServerConfig{...})` first (bus, logs, nexus, pricing, `ResolveLLM`/`ModelNames` closures, bash-allow store, cwd, trust default, debug — all collaborators typed as interfaces declared in `api/api.go`; the optional `Nexus`/`BashAllow` fields are left unset rather than given a nil pointer; the cost tracker must exist before event forwarding starts), then `harness.New` with the server's `TextDelta`/`ThinkingDelta` callbacks as delta handlers, then `srv.Attach(h)`. The container keeps ownership of the harness and bus and shuts them down after `srv.Shutdown`. `runServe` calls `signal.Reset(os.Interrupt)` after the server stops, so a second Ctrl+C during shutdown force-kills the process instead of being swallowed by the `NotifyContext` registered in `main`.
...
A line matching `error_pattern` emits a `ChannelErrorEvent` and, if the channel has `trigger` enabled, notifies a `Trigger`. `Trigger` (`internal/app/nexus/trigger.go`) debounces per channel (30s cooldown, wired in `cmd/app/container.go`) and holds a global queue-of-one: while a turn is running, newly erroring channels accumulate in a pending set instead of firing again; `TurnEnded()` (called when the turn queue drains via `api.ServerConfig.OnTurnEnd` → `turnqueue.QueueConfig.OnTurnEnd`) flushes the pending set once the agent is free. A successful wake starts a turn with a synthesized investigation prompt (`Server.nexusPrompt`, `api/turns.go`) built from each pending channel's recent errors.
...
Three nexus event types are forwarded over SSE by `Server.forwardEvents`, mapped via the wire schema like every other forwarded event: `nexus.channel_error` (channel, text, seq), `nexus.channel_status` (channel, state — channel source goroutine lifecycle), and `nexus.trigger` (channels — emitted when a wake actually starts a turn).
...
Lives in-repo under `pkg/`. `pkg/common` holds the canonical types; `pkg/providers/protocols/` holds the protocol clients, each `NewClient(model common.Model, opts...) (common.LLM, error)`: `anthropic` (native SDK), `ollama` (native `/api/*` endpoints), and `openai_compat` (OpenAI, OpenRouter, or any OpenAI-compatible endpoint via `WithBaseURL`). `systemone` is the exception: it implements `common.SystemOne` (typed questions in, calibrated typed answers out — no text, no tools, no streaming) rather than `common.LLM`, and `NewClient(opts...)` takes no model value. Models come from `pkg/models` or any `common.Model` implementation. API keys/base URLs are caller-supplied via client options — nothing in `pkg/` reads env vars. The harness never constructs clients; `internal/app/modelregistry` is the app-side factory (`buildLLM` switches on the model's provider type — one of three: anthropic, ollama, openai_compat — and takes the url/key from that provider entry, with no env-var fallback) with a per-run `Factory` cache injected by `cmd/app/deps.go`. The anthropic client takes `WithBaseURL` like the others, so a declared `url:` is honoured rather than ignored, and an empty one leaves each client's own default in place. Client-side limiting is uniform across protocols and internal to `NewClient`: `WithRateLimit(ratelimit.TokenBucketConfig)` (API rate) and `WithMaxConcurrency(n)` (concurrency) are independent options, both off by default — callers never call `ratelimit.Wrap` themselves.
...
- Cost budgets in USD at the harness level — the API server tracks USD cost from tenzing.yaml pricing (`api/costs`, `GET /stats`), but `budgets.Limits` has no `MaxCost` field.
