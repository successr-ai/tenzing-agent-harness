...
- **Models:** `--model` (default `ollama/glm-5.2:cloud`), `--subagent-model`, `--blackboard-model`, `--advisor-model` (enables the advisor tool + write-gate; `--advisor-nudge N` optionally reminds an unconsulted executor from iteration N) — resolved via `cfg.deps`' registry (see "Model registry" below)
